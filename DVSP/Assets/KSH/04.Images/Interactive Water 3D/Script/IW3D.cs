...
namespace InteractiveWater3D
{
	...
	public class IW3D : MonoBehaviour
	{
		...
		[Range(0.01f, 0.1f)] public float m_HeightScale = 0.01f;
		...
		void Start()
		{
			...
			Mesh mesh = IW3DUtils.CreateGridPlaneMesh(m_Grid, m_Width, m_Width, new Vector3(-m_Width / 2f, 0, -m_Width / 2f));
			MeshFilter filter = m_WaterPlane.AddComponent<MeshFilter>();
			filter.mesh = mesh;
			MeshRenderer rd = m_WaterPlane.AddComponent<MeshRenderer>();
			rd.material = m_MatShading;

			m_CamOrth = IW3DUtils.CreateOrthographicCamera(m_WaterPlane.transform, m_WaterForceLayer, m_SdrPush, m_Width / 2f);
			CreateInternalMaps();
		...
		void OnDestroy()
		{
			for (int i = 0; i < m_RTHeightmaps.Length; i++)
			{
				if (m_RTHeightmaps[i])
				{
					Destroy(m_RTHeightmaps[i]);
					m_RTHeightmaps[i] = null;
				}
			}
			if (m_RTNormal)
			{
			...
			if (m_RTReflection)
			{
				Destroy(m_RTReflection);
				m_RTReflection = null;
			}
			if (m_RTRefraction)
			{
				Destroy(m_RTRefraction);
				m_RTRefraction = null;
			}
		}
		...
		void OnGUI()
		{
			GUI.Box(new Rect(5, 5, 160, 22), "Interactive Water 3D");
		...
		void CreateInternalMaps()
		{
			...
			m_MatHeightToNormal.SetFloat("_InvRadius", 300f);

			m_RTHeightmaps[0] = new RenderTexture(m_Resolution, m_Resolution, 0, rtf);
			...
			m_RTHeightmaps[1] = new RenderTexture(m_Resolution, m_Resolution, 0, rtf);
			...
			m_RTHeightmaps[2] = new RenderTexture(m_Resolution, m_Resolution, 0, rtf);
