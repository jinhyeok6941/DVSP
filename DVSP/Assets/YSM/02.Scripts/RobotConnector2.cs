...
public class RobotConnector2 : MonoBehaviour
{
    ...
    //public EDroneType _eCurDroneType = EDroneType.CODRONE2; // 추후 드론 확장을 위해
...
//    public int  mode_controlPressed = 0;
    ...
    //public EController _eController = EController.NONE; // 접속된 컨트롤러 
    ...
    void GetVersion(uint p_Model, byte p_Major, byte p_Minor, ushort p_Build)
    {
        ...
        if ((p_Model >= 270338) && (p_Model <= 270350)) // 코드론 II 조종기
        {
            ...
            //_eController = EController.CONTROLLER_C2;
        }
        else if ((p_Model >= 204803) && (p_Model <= 204810)) // 코드론 미니 조종기
        {
            ...
            _nC_Version_major = p_Major;
            _nC_Version_minor = p_Minor;
            //_eController = EController.CONTROLLER_CM;
        }
        else if ((p_Model >= 208896) && (p_Model <= 208910)) // RF 모듈
        {
            ...
            _nC_Version_major = p_Major;
            _nC_Version_minor = p_Minor;
            //_eController = EController.RF_MODULE;
        }
        ...
        {
            *//*if (_eController == EController.RF_MODULE)
                serController = "RFM  v ";*//*
            else
                serController = "C  v ";
            
            _main._lblSelectedBLE_Name.text = serController + _nC_Version_major.ToString() + "." + _nC_Version_minor.ToString() + "." + _nC_Version_build.ToString() + "       D  v " +
                                                              //_nD_Version_major.ToString() + "." + _nD_Version_minor.ToString() + "." + _nD_Version_build.ToString();
        }*/
    }
    ...
    void Update()
    {
        ﻿if (_opened == true)
        {
            ...
            if (tempBytes != null)
            {
                ...
                //if (_eCurDroneType == EDroneType.CODRONE2)
                {
                    if ((tempBytes[0] == 0x0A)&&(tempBytes[1] == 0x55))
                    {
                        ...
                        //                        Debug.Log(" [0]: " + Convert.ToString(readBytes[0], 16) +
                        //                            " [1]: " + Convert.ToString(readBytes[1], 16) +
                        //                            " [2]: " + Convert.ToString(readBytes[2], 16) +
                        //                            " [3]: " + Convert.ToString(readBytes[3], 16) +
                        //                            " [4]: " + Convert.ToString(readBytes[4], 16) +
                        //                            " [5]: " + Convert.ToString(readBytes[5], 16));
                        ...
                        if((crcL == readBytes[readBytes.Length - 2])&&(crcH == readBytes[readBytes.Length - 1]))
                        {
                            switch (readBytes[2])
                            {
                                case (byte)Protocol.DataType.Type.State:
                                    //Debug.Log("Case: State" + "        " + Convert.ToString(state.modeFlight, 16) );
                                    state.modeSystem = readBytes[6];               //mode.sysyem.Type 참조
                                    state.modeFlight = readBytes[7];               //mode.Flight.Type 참조
                                    state.modeControlFlight = readBytes[8];       //mode.control.flight.Type 참조
                                    state.modeMovement = readBytes[9];            //mode.Movement.Type 참조
                                    state.headless = readBytes[10];                //mode.heddless.Type 참조
                                    state.controlSpeed = readBytes[11];            //mode.controlSpeed.Type 참조
                                    state.sensorOrientation = readBytes[12];       //mode.SensorOrientation.Type 참조
                                    state.battery = readBytes[13];                 //definition이 불필요(0 ~ 100)
                                    //_main.ReceivePacket(); // 수신상태 표시
                                    break;

                                case (byte)Protocol.DataType.Type.Attitude:
                                    //Debug.Log("Case: Attitude");
                                    attitude.roll = BitConverter.ToInt16(readBytes, 6);
                                    attitude.pitch = BitConverter.ToInt16(readBytes, 8) ;
                                    attitude.yaw = BitConverter.ToInt16(readBytes, 10);
                                    //_main.ReceivePacket(); // 수신상태 표시
                                    break;

                                case (byte)Protocol.DataType.Type.Altitude:
                                    //Debug.Log("Case: Altitude");
                                    altitude.temperature = BitConverter.ToSingle(readBytes, 6);
                                    altitude.pressure = BitConverter.ToSingle(readBytes, 10);
                                    altitude.altitude = BitConverter.ToSingle(readBytes, 14);
                                    altitude.rangeHeight = BitConverter.ToSingle(readBytes, 18);
                                    //_main.ReceivePacket(); // 수신상태 표시
                                    break;

                                case (byte)Protocol.DataType.Type.Motion:
                                    //Debug.Log("Case: Motion");
                                ...
                                case (byte)Protocol.DataType.Type.RawFlow:
                                    //Debug.Log("Case: Flow");
                                    flow.x = BitConverter.ToSingle(readBytes, 6);
                                    flow.y = BitConverter.ToSingle(readBytes, 10);
                                    //_main.ReceivePacket(); // 수신상태 표시
                                    break;

                                case (byte)Protocol.DataType.Type.Range:
                                    //Debug.Log("Case: Range");
                                    range.left = BitConverter.ToInt16(readBytes, 6);
                                    range.front = BitConverter.ToInt16(readBytes, 8);
                                    range.right = BitConverter.ToInt16(readBytes, 10);
                                    range.rear = BitConverter.ToInt16(readBytes, 12);
                                    range.top = BitConverter.ToInt16(readBytes, 14);
                                    range.bottom = BitConverter.ToInt16(readBytes, 16);
                                    //_main.ReceivePacket(); // 수신상태 표시
                                    break;

                                case (byte)Protocol.DataType.Type.Position:
                                    //Debug.Log("Case: Position");
                                    position.x = BitConverter.ToSingle(readBytes, 6);
                                    position.y = BitConverter.ToSingle(readBytes, 10);
                                    position.z = BitConverter.ToSingle(readBytes, 14);
                                    //_main.ReceivePacket(); // 수신상태 표시
                                    break;

                                case (byte)Protocol.DataType.Type.Information:
                                    _nModelNumber = BitConverter.ToUInt32(readBytes, 7); // 모델 번호
                                    _usVersion_Build = BitConverter.ToUInt16(readBytes, 11); // 0 ~ 65535  빌드 번호 14 bit
                                    _byVersion_Minor = readBytes[13]; // 0 ~ 255    부 번호   1 Byte
                                    _byVersion_Major = readBytes[14]; // 0 ~ 255    주 번호   1 Byte
                                    _usDate_Year = BitConverter.ToUInt16(readBytes, 15); // 0 ~ 65535  펌웨어 빌드 년
                                    _byDate_Month = readBytes[17]; // 0 ~ 255    펌웨어 빌드 월
                                    _byDate_Day = readBytes[18]; // 0 ~ 255    펌웨어 빌드 일

                                    GetVersion(_nModelNumber, _byVersion_Major, _byVersion_Minor, _usVersion_Build); // 버젼 받기
                                    //_main.ReceivePacket(); // 수신상태 표시
                                    break;

                                case (byte)Protocol.DataType.Type.Trim:
                                    //Debug.Log("Case: Trim");
                                    trim.roll = BitConverter.ToInt16(readBytes, 6);
                                    trim.pitch = BitConverter.ToInt16(readBytes, 8);
                                    trim.yaw = BitConverter.ToInt16(readBytes, 10);
                                    trim.throttle = BitConverter.ToInt16(readBytes, 12);
                                    //_main.ReceivePacket(); // 수신상태 표시
                                    break;

                                case (byte)Protocol.DataType.Type.Count:
                                    //Debug.Log("Case: Count");
                                    count.timeSystem = BitConverter.ToUInt32(readBytes, 6);
                                    count.timeFlight = BitConverter.ToUInt32(readBytes, 10);
                                    count.countTakeOff = BitConverter.ToUInt16(readBytes, 14);
                                    count.countLanding = BitConverter.ToUInt16(readBytes, 16);
                                    count.countAccident = BitConverter.ToUInt16(readBytes, 18);
                                    //_main._CoDronSensorManager.Show_Count(count.timeSystem, count.timeFlight, count.countTakeOff, count.countLanding, count.countAccident);
                                    //_main.ReceivePacket(); // 수신상태 표시
                                    break;
...
//                }
    ...
    public void Disconnect()
    {
        ...
        //_main._lblSelectedBLE_Name.text = "";
